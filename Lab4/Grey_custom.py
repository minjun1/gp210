...
class Grey(sepPlot_custom.sepPlot):
	...
	def setDefaults(self):
		o = super().setDefaults()
		if 'bpclip' in self.kw: o.addParam("bpclip", "begin clip",self.kw.get('bpclip'))
		else: o.addParam("bpclip", "begin clip",1.)
		if 'epclip' in self.kw: o.addParam("epclip", "end clip",self.kw.get('epclip'))
		else: o.addParam("epclip", "end clip",99.)
	...
	def output(self):
		...
		if len(self.dims)==2:
			...
			self.plt.imshow(mat,cmap=self.getColormap(),extent=self.getAxisLimits(),aspect=self.getParam("aspect"))
...
#					im=self.plt.imshow(mat,animated=True)
					im=self.plt.imshow(mat,cmap=self.getColormap(),animated=True,
