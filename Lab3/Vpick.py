...
class Vpick:
    ...
    def create_vels(self,data,vmin,vmax,dv):
        nv = int((vmax-vmin)/dv)
        vels = SepVector.get_sep_vector(Hypercube.set_with_ns(ns=[data.get_hyper().get_axis(1).n,nv],
                                                    ds=[data.get_hyper().get_axis(1).d,dv],
                                                    os=[data.get_hyper().get_axis(1).o,vmin],
